...
- **クイックスキャフォールディング (+ Git/GitHub連携 & UIVerse対応)**: `+ New Project` を使って、`.agents`、規範ドキュメント、`.gitignore`、ローカルGit初期化、リモートURL連携、およびGitHubリポジトリ自動作成（VS Code GitHub認証 / `gh` CLI対応、優雅なフォールバック付き）、UIVerse.io Design System（`DESIGN.md` / `system.css` / `preview.html`）の初期化を即座に実行します。
...
- **クラウド＆プロバイダーヘルパー (`⎔ Cloud & Provider Helper`)**: Firebase（承認済みドメイン、Web API設定、Firestore/Storageルール）、Google Cloud（OAuth同意画面、認証情報、Secret Manager、IAM）、Supabase（APIキー、リダイレクトURL、RLSポリシー）、Wix Studio/Velo、Vercel/Netlify、GitHub、AIプラットフォーム（OpenAI、Claude、Gemini）、Stripeなどの見つけにくい設定画面へワンクリックでアクセスできるダイレクトナビゲーター。プロジェクトIDを動的に挿入し、コンソール画面へ直接ジャンプ可能です。
...
- **Cloud & Provider Helper (`⎔ Cloud & Provider Helper`)**: Direct deep links to hard-to-find developer console settings across Firebase (Authorized Domains, Web API keys, Firestore/Storage rules), Google Cloud Platform (OAuth Consent Screen, API Credentials, Secret Manager, IAM), Supabase (API keys, Site/Redirect URLs, RLS Policies), Wix Studio/Velo, Vercel, Netlify, GitHub, AI Platforms (OpenAI, Claude, Gemini), and Stripe. Includes dynamic Project ID URL substitution to jump directly into your active project console.
...
- **Project Scaffolding (+ Git & GitHub Integration)**: Instantly generate complete project foundations with `+ New Project`, creating `.agents/`, canonical `docs/core/`, secret-protected `.gitignore`, baseline Git initialization, remote URL linking, or automated GitHub repository creation (via VS Code GitHub Auth / `gh` CLI with graceful fallbacks), and optional UIVerse.io Design System assets.
