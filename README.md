...
1. VCSサイドバーの **+ New Project** をクリックして、`.agents`、`docs/core`、および `.gitignore` を自動生成します（または保管庫から **スキャフォールドテンプレート** を選択して即座に設定と同梱アセットを展開）。
2. **⏻ Agent Initialization Prompt** をクリックして初期化プロンプトをコピーし、AIチャットに貼り付けてエージェントを接地します。
...
- **Community Edition (無料 / OSS)**: 100%オフライン、ローカルRAG、ローカルAES-256暗号化Vault、3フェーズワークフロー、スキャフォールドテンプレート保管庫。
...
- **Permanent Indexing CLI Tool (`npm run vibe:sync`)**: One-command synchronization via `node .agents/vibe-sync.js` that updates `.vibe-rag.json` and `.vibe-tracker.md` while preserving scratch notes and vault secrets.
...
1. Click **+ New Project** in the VCS Sidebar to scaffold `.agents/`, `docs/core/`, `.gitignore`, and Git version control (or select a **Scaffold Template** from the Storehouse to instantly prefill settings and bundled assets).
2. Click **⏻ Agent Initialization Prompt** in the sidebar and paste into your AI agent's chat to ground it.
