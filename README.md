...
- **クイックスキャフォールディング (+ Git自動初期化)**: `+ New Project` を使って、`.agents`、規範ドキュメント、`.gitignore`、およびGitリポジトリの初期化を即座に実行します。
...
1. VCSサイドバーの **+ New Project** をクリックして、`.agents`、`docs/core`、および `.gitignore` を自動生成します。
...
- **Community Edition (無料 / OSS)**: 100%オフライン、ローカルRAG、ローカルAES-256暗号化Vault、3フェーズワークフロー。
...
1. Click **+ New Project** in the VCS Sidebar to scaffold `.agents/`, `docs/core/`, `.gitignore`, and Git version control.
